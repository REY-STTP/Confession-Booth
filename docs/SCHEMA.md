...
| wallet_first_tx_at | timestamptz | nullable, first on-chain interaction (wallet age proxy) |
...
| ----------------- | -------------------- | ----------------------------------------------------------------------------- |
| id                | uuid                 | PK                                                                            |
| public_id         | varchar              | Anonymous-facing ID (`c_...`)                                                 |
| author_user_id    | uuid                 | private FK                                                                    |
| category_id       | uuid                 | FK                                                                            |
| content_object_id | uuid                 | FK                                                                            |
| status            | enum                 | PENDING/VISIBLE/QUARANTINED/HIDDEN/REMOVED (QUARANTINED = triage kritis auto) |
...
| version           | integer              |                                                                               |
| moderation_score  | numeric(5,2)         | private, precision 5 scale 2 (cap 100.00)                                     |
...
**Index:** `CREATE INDEX idx_ratelimit_window ON rate_limit_buckets (window_start);`
...
## 15b. idempotency_keys

| Column      | Type         |
| ----------- | ------------ |
| key         | varchar(128) | PK (unique per user) |
| user_id     | uuid         | FK                   |
| status_code | integer      |
| response    | jsonb        |
| created_at  | timestamptz  |
...
Unique constraint: `(key, user_id)` — TTL 24 jam.
...
## 15d. indexer_mismatches

| Column           | Type          |
| ---------------- | ------------- |
| id               | uuid          | PK            |
| confession_id    | varchar(66)   |
| onchain_hash     | varchar(128)  |
| db_hash          | varchar(128)  |
| block_number     | bigint        |
| tx_hash          | varchar(66)   |
...
Indexes: `idx_indexer_mismatches_confession`, `idx_indexer_mismatches_detected`, `idx_indexer_mismatches_unresolved`
...
VISIBLE
  ...
  ├── critical report (THREAT/DOXXING/SEXUAL_EXPLOITATION ≥3) → QUARANTINED
  └── QUARANTINED → human review → HIDDEN | REMOVED | RESTORE → VISIBLE
```
