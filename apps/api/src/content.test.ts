...
async function promote(addr: string, role: 'MODERATOR' | 'ADMIN' = 'MODERATOR') {
  await db.execute(sql`UPDATE users SET role = ${role} WHERE wallet_address = ${addr.toLowerCase()}`);
}
...
describe('POST /confessions (T1-010)', () => {
  ...
  it('rate-limit: confession valid ke-4 → 429 (400 tidak dihitung)', async () => {
    ...
    for (let i = 0; i < 3; i++) assert.equal((await publish(u.token, 'life', rnd(`kuota ${i}`))).statusCode, 201);
...
describe('reactions (T1-012)', () => {
  it('post + idempoten + unlike + counts di detail', async () => {
    ...
    const anon = await app.inject({ method: 'POST', url: `/api/confessions/${pid}/reactions`, payload: { type: 'LOVE' } });
    assert.equal(anon.statusCode, 401);

    const r1 = await app.inject({
      method: 'POST', url: `/api/confessions/${pid}/reactions`, remoteAddress: ip(),
      headers: { authorization: `Bearer ${reactor.token}` }, payload: { type: 'LOVE' },
    });
    ...
    const bad = await app.inject({
      method: 'POST', url: `/api/confessions/${pid}/reactions`, remoteAddress: ip(),
      headers: { authorization: `Bearer ${reactor.token}` }, payload: { type: 'LIKE' },
    });
    ...
    const del = await app.inject({
      method: 'DELETE', url: `/api/confessions/${pid}/reactions/LOVE`,
...
describe('whispers (T1-013)', () => {
  it('post + list + whisperCount + XSS 400 + dup 409', async () => {
    ...
    const xss = await app.inject({
      method: 'POST', url: `/api/confessions/${pid}/whispers`, remoteAddress: ip(),
      headers: { authorization: `Bearer ${w.token}` }, payload: { content: '<img src=x onerror=1>' },
    });
    ...
    const p1 = await app.inject({
      method: 'POST', url: `/api/confessions/${pid}/whispers`, remoteAddress: ip(),
      headers: { authorization: `Bearer ${w.token}` }, payload: { content: text },
    });
...
describe('reports + moderation (T1-014/T1-015)', () => {
  it('report anonim OPEN; reason invalid 404/400; masuk antrean moderator', async () => {
    ...
    const badReason = await app.inject({ method: 'POST', url: '/api/reports', payload: { targetType: 'CONFESSION', targetId: pid, reason: 'RUDE' } });
    assert.equal(badReason.statusCode, 400);
    const badTarget = await app.inject({ method: 'POST', url: '/api/reports', payload: { targetType: 'CONFESSION', targetId: 'c_tidakada', reason: 'SPAM' } });
    assert.equal(badTarget.statusCode, 404);

    const rep = await app.inject({
      method: 'POST', url: '/api/reports', remoteAddress: ip(),
    ...
    assert.equal((await app.inject({ method: 'GET', url: `/api/confessions/${pid}` })).statusCode, 200);

    const q = await app.inject({ method: 'GET', url: '/api/moderation/queue?status=OPEN', headers: { authorization: `Bearer ${mod.token}` } });
  ...
  it('konten kritis → QUARANTINED + hilang dari feed; hide → audit; restore → kembali', async () => {
    ...
    const rep = await app.inject({ method: 'POST', url: '/api/reports', remoteAddress: ip(), payload: { targetType: 'CONFESSION', targetId: pid, reason: 'DOXXING' } });
    assert.equal(rep.statusCode, 201);
    assert.equal((await app.inject({ method: 'GET', url: `/api/confessions/${pid}` })).statusCode, 404);

    const userHide = await app.inject({
      method: 'POST', url: '/api/moderation/actions', headers: { authorization: `Bearer ${author.token}` },
      payload: { targetType: 'CONFESSION', targetId: pid, action: 'HIDE', reason_code: 'DOXXING', policy_version: 'v1.0' },
    });
    assert.equal(userHide.statusCode, 403);

    const hide = await app.inject({
      method: 'POST', url: '/api/moderation/actions', headers: { authorization: `Bearer ${mod.token}` },
      payload: { targetType: 'CONFESSION', targetId: pid, action: 'HIDE', reason_code: 'DOXXING', policy_version: 'v1.0' },
    });
    ...
    const restore = await app.inject({
      method: 'POST', url: '/api/moderation/actions', headers: { authorization: `Bearer ${mod.token}` },
      payload: { targetType: 'CONFESSION', targetId: pid, action: 'RESTORE', reason_code: 'OTHER', policy_version: 'v1.0' },
    });
  ...
  it('BAN author → tulis diblokir 403', async () => {
    ...
    const ban = await app.inject({
      method: 'POST', url: '/api/moderation/actions', headers: { authorization: `Bearer ${mod.token}` },
      payload: { targetType: 'CONFESSION', targetId: pid, action: 'BAN', reason_code: 'HARASSMENT', policy_version: 'v1.0' },
    });
  ...
  it('semua respons publik bebas identitas', async () => {
    ...
    for (const url of urls) {
      ...
      if (url.endsWith('/proof')) {
        ...
        assert.ok(typeof body.contractAddress === 'string' && body.contractAddress.startsWith('0x'), url);
