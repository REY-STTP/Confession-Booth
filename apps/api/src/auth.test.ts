...
before(async () => {
  await app.ready();
  await db.execute(sql`TRUNCATE sessions, auth_nonces, users, rate_limit_buckets RESTART IDENTITY CASCADE`);
});
...
describe('auth wallet (T1-002)', () => {
  ...
  it('replay nonce ditolak (NONCE_REUSED)', async () => {
    const ip = '10.10.0.12';
    const nRes = await app.inject({ method: 'GET', url: `/api/auth/nonce?address=${A.address}`, remoteAddress: ip });
    const { nonce, message } = nRes.json();
    const sig = await A.signMessage({ message });
    const first = await app.inject({ method: 'POST', url: '/api/auth/verify', remoteAddress: ip, payload: { address: A.address, signature: sig, nonce } });
    assert.equal(first.statusCode, 200);
    const replay = await app.inject({ method: 'POST', url: '/api/auth/verify', remoteAddress: ip, payload: { address: A.address, signature: sig, nonce } });
  ...
  it('signature orang lain ditolak (BAD_SIGNATURE)', async () => {
    ...
    const res = await app.inject({ method: 'POST', url: '/api/auth/verify', remoteAddress: ip, payload: { address: A.address, signature: evil, nonce } });
  ...
  it('nonce kedaluwarsa ditolak (NONCE_EXPIRED)', async () => {
    ...
    const nRes = await app.inject({ method: 'GET', url: `/api/auth/nonce?address=${A.address}`, remoteAddress: ip });
    const { nonce, message } = nRes.json();
    await db.execute(sql`UPDATE auth_nonces SET expires_at = now() - interval '1 minute' WHERE nonce_hash = ${sha256hex(nonce)}`);
    const sig = await A.signMessage({ message });
    const res = await app.inject({ method: 'POST', url: '/api/auth/verify', remoteAddress: ip, payload: { address: A.address, signature: sig, nonce } });
    assert.equal(res.statusCode, 401);
  ...
  it('domain salah ditolak (WRONG_DOMAIN)', async () => {
    ...
    const nRes = await app.inject({ method: 'GET', url: `/api/auth/nonce?address=${A.address}`, remoteAddress: ip });
    const { nonce, message } = nRes.json();
    await db.execute(sql`UPDATE auth_nonces SET domain = 'evil.test' WHERE nonce_hash = ${sha256hex(nonce)}`);
    const sig = await A.signMessage({ message });
    const res = await app.inject({ method: 'POST', url: '/api/auth/verify', remoteAddress: ip, payload: { address: A.address, signature: sig, nonce } });
    assert.equal(res.statusCode, 401);
  ...
  it('address tanpa signature / mismatch ditolak', async () => {
    const ip = '10.10.0.16';
    const noSig = await app.inject({ method: 'POST', url: '/api/auth/verify', remoteAddress: ip, payload: { address: A.address, nonce: 'x' } });
    assert.equal(noSig.statusCode, 400);
    const nRes = await app.inject({ method: 'GET', url: `/api/auth/nonce?address=${A.address}`, remoteAddress: ip });
    const { nonce } = nRes.json();
    const mismatch = await app.inject({
      method: 'POST', url: '/api/auth/verify', remoteAddress: ip,
  ...
  it('address disimpan lowercase + user upsert (login 2x = 1 user)', async () => {
    ...
    const found = rowsOf<{ wallet_address: string }>(
      await db.execute(sql`SELECT wallet_address FROM users WHERE wallet_address = ${B.address.toLowerCase()}`),
    );
  ...
  it('refresh rotation + logout revoke', async () => {
    ...
    const r1 = await app.inject({ method: 'POST', url: '/api/auth/refresh', remoteAddress: ip, headers: { cookie } });
    assert.equal(r1.statusCode, 200);
    assert.ok(r1.json().accessToken);
    // refresh lama tidak bisa dipakai lagi
    const r2 = await app.inject({ method: 'POST', url: '/api/auth/refresh', remoteAddress: ip, headers: { cookie } });
    assert.equal(r2.statusCode, 401);
    // logout revoke access token
    const access = vRes.json().accessToken;
    const lo = await app.inject({ method: 'POST', url: '/api/auth/logout', remoteAddress: ip, headers: { authorization: `Bearer ${access}`, cookie } });
  ...
  it('rate-limit nonce 10/10 mnt + header Retry-After', async () => {
    const ip = '10.10.0.20';
    for (let i = 0; i < 10; i++) {
      const r = await app.inject({ method: 'GET', url: `/api/auth/nonce?address=${A.address}`, remoteAddress: ip });
      assert.equal(r.statusCode, 200);
    }
    const over = await app.inject({ method: 'GET', url: `/api/auth/nonce?address=${A.address}`, remoteAddress: ip });
...
describe('session + RBAC (T1-003)', () => {
  it('anon 401, user biasa 403 di /moderation/queue', async () => {
    ...
    const user = await app.inject({
      method: 'GET', url: '/api/moderation/queue',
  ...
  it('moderator 200; akun BANNED diblokir', async () => {
    ...
    const access = vRes.json().accessToken;
    await db.execute(sql`UPDATE users SET role = 'MODERATOR' WHERE wallet_address = ${B.address.toLowerCase()}`);
    const mod = await app.inject({ method: 'GET', url: '/api/moderation/queue', headers: { authorization: `Bearer ${access}` } });
    assert.equal(mod.statusCode, 200);
    await db.execute(sql`UPDATE users SET status = 'BANNED' WHERE wallet_address = ${B.address.toLowerCase()}`);
    const banned = await app.inject({ method: 'GET', url: '/api/moderation/queue', headers: { authorization: `Bearer ${access}` } });
    assert.equal(banned.statusCode, 403);
    await db.execute(sql`UPDATE users SET status = 'ACTIVE', role = 'USER' WHERE wallet_address = ${B.address.toLowerCase()}`);
  });

  it('token palsu → anon (tidak bocor info)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/moderation/queue', headers: { authorization: 'Bearer ffff' } });
    assert.equal(res.statusCode, 401);
  });
});
